...
* **main.py:** 사용자 입력 처리 및 터미널 UI 렌더링을 담당하는 엔트리 포인트입니다.
  ...
  ```
  **📝 전문 용어(아키텍처 개념) 해설:**
  * **인스턴스화 (Instantiation):** `game = QuizGame()` 부분으로, 추상적인 설계도(클래스)를 바탕으로 실제 메모리상에 작동하는 실체(객체)를 만들어내는 과정입니다. 비유하자면 **'식당 오픈 전에 설계도를 보고 주방(시스템)을 실제 건물에 세팅하는 것'**과 같습니다.
  * **UI 렌더링 (UI Rendering):** `game.print_menu()` 부분으로, 사용자에게 보여줄 텍스트나 메뉴판을 화면(터미널)에 실제로 그려내는(출력하는) 작업입니다.
  * **제어권 위임 (Delegation):** 사용자가 메뉴를 선택했을 때 `main.py`가 직접 채점을 하는 것이 아니라, "나는 주문만 받을 테니 요리는 네가 해!" 하고 `quiz.py`(로직 계층)의 함수를 호출하여 일처리와 통제권을 넘겨주는 객체 지향의 핵심 설계 방식입니다.
