...
> [!TIP]
> **💡 객체 지향 프로그래밍(OOP)이 아직 낯설게 느껴지시나요?**
> 쉽게 말해 클래스(Class)는 붕어빵 틀, 객체(Object)는 그 틀로 찍어낸 붕어빵에 비유할 수 있습니다. 위 코드에서 `Quiz`는 퀴즈 데이터의 '틀'이고, `QuizGame`은 퀴즈를 운영하는 '매니저' 역할을 합니다. 객체 지향의 4대 원칙(캡슐화, 상속, 다형성, 추상화)을 이해하고 싶다면 아래 **개발자 기술 블로그**를 참고해 보세요!
> 
> 📚 **추천 기술 블로그 (Velog):**
